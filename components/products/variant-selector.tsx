...
import { getShopifyProductId } from '@/lib/utils';
      ...
      <dd className="flex flex-wrap gap-2">
        {option.values.map(value => {
          ...
          const filtered = Object.entries(optionParams).filter(([key, value]) =>
            options.find(option => option.name.toLowerCase() === key && option.values.some(val => val === value))
          );
          const isAvailableForSale = combinations.find(combination =>
            filtered.every(([key, value]) => combination[key] === value && combination.availableForSale)
          );

          const isActive = isTargetingProduct && selectedValue === value;
...
export const useProductImages = (product: Product | CartProduct, selectedOptions?: SelectedOptions) => {
  ...
  const optionsObject = useMemo(() => {
    ...
    // selectedOptions is a Record<string, string>, so we just need to normalize keys/values
